...
$(document).ready(function () {
  ...
  //global variables
  action_name = "info_form";
  let randNum = Math.floor(Math.random() * 1000000);
  user_id = "bsingh" + randNum;
...
function callApi() {
  ...
  $.ajax({
    ...
    success: function (botResponse, status) {
      ...
      setBotResponse(botResponse, true);
...
function setBotResponse(response, quantity = false) {
  ...
  setTimeout(function () {
    ...
    if (response.length < 1) {
      ...
      var BotResponse =
        '<img class="botAvatar" src="./static/img/logo2.jpeg"/><p class="botMsg">' +
        fallbackMsg +
        '</p><div class="clearfix"></div>';
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        if (response[i].hasOwnProperty("custom")) {
          ...
          if (response[i].custom.payload == "cardsCarousel") {
            ...
            showCardsCarousel(restaurantsData, quantity);
          }
          if (response[i].custom.payload == "itemsCarousel") {
            $("#paginated_cards").remove();
            restaurantsData = response[i].custom.data;
            showItemsCarousel(restaurantsData);
          }

          if (response[i].custom.payload == "buttons") {
            data = response[i].custom.notes;
            backButtons(data);
          }
...
function showCardsCarousel(cardsToAdd, quantity) {
  var cards = createCardsCarousel(cardsToAdd, quantity);

  $(cards).appendTo(".chats").hide().fadeIn(1000);
...
function createCardsCarousel(cardsData, quantity) {
  ...
  for (i = 0; i < cardsData.length; i++) {
    ...
    if (quantity === false) {
      ...
      <div class="carousel_cards in-left">
        <a href="${data.pageSrc}" target="_blank"><img class="cardBackgroundImage" src="${data.imgSrc}" /></a>
        <div class="cardFooter">
          <div class="cardTitle" title="${data.name}">
            <a href="${data.pageSrc}" target="_blank">${data.name}</a>
            <p>${data.price} INR</p>
          </div>
        </div>
      </div>`;
...
function showItemsCarousel(cardsToAdd) {
  var cards = createItemsCarousel(cardsToAdd);
...
function createItemsCarousel(itemData) {
  ...
  for (i = 0; i < itemData.length; i++) {
    ...
    for (let j = 0; j < data.length; j++) {
      details += `
      <p style="opacity: 0.7; font-size: 12px;">${data[j].Date}<p>
      <p>
      Price:${data[j].price} </br>
      Order ID:${data[j].order_id} </br>
      ${data[j].Items} Items </br>
      ${data[j].status}
      </p>
      `;
    }
    ...
    console.table(data);
    item = `
    <div class="carousel_cards in-left itemCarouselContent">
      <div class="card-content" >
        ${details}
        <a class="waves-effect waves-light btn" style="font-size:11px; background:#fbc22d;color:black;" onclick="callApi()">Manage Order</a>
      </div>
    </div>
    `;
  ...
  var cardContents =
    '<div id="paginated_cards" class="cards"> <div class="cards_scroller itemCardsScroller">' +
    cards +
    '  <span class="arrow prev fa fa-chevron-circle-left "></span> <span class="arrow next fa fa-chevron-circle-right" ></span> </div> </div>';
...
function showBotTyping() {
  var botTyping =
    '<img class="botAvatar" id="botAvatar" src="./static/img/logo2.jpeg"/><div class="botTyping">' +
...
const backBtnClickHandler = (message) => {
  console.log(message);
  setUserResponse(message);
  send(message);
};

function backButtons(textToAdd) {
  console.log(textToAdd);
  setTimeout(function () {
    const data = textToAdd;

    let btns = ``;

    for (i = 0; i < data.length; i++) {
      if (data[i].resp === "cancel" || data[i].resp === "return") {
        btns += `<a class="waves-effect waves-light btn-small" style="margin:4px;" href="http://www.fluorite.sa" target="_blank">${data[i].name}</a>`;
      } else {
        btns += `<a class="waves-effect waves-light btn-small" style="margin:4px;" onclick="backBtnClickHandler('${data[i].resp}')">${data[i].name}</a>`;
      }
    }

    let BotResponse = `
      <div>
        <img class="botAvatar" src="./static/img/logo2.jpeg" />
          <div class="backBtnContainer">${btns}</div>
...
const collapsibleBtnClickHandler = (message) => {
  setUserResponse(message);
  send(message);
  $(".collapsible").remove();
};
...
function createCollapsible(data) {
  ...
  for (i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < data[i].notes.length; j++) {
      btns += `
        <div>
          <a class="waves-effect waves-light btn inCollapsibleBtn" onclick="collapsibleBtnClickHandler('${data[i].notes[j].resp}')">${data[i].notes[j].name}</a>
        </div>`;
    }
    item =
      "<li>" +
